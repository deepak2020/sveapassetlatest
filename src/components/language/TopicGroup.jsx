...
export default function TopicGroup({ topic, lessons, index }) {
    ...
    >
      <div className="px-6 py-4 bg-muted/40 border-b border-border/50 flex items-center justify-between">
        <div>
          ...
          {topicSv !== topic && (
            <p className="text-xs italic text-muted-foreground">{topicSv}</p>
          )}
        </div>
        ...
        {sorted.map((lesson) => {
          ...
          return (
            <Link key={lesson.id} to={`/language/${lesson.id}`} className="group">
              <div className={`h-full rounded-xl border ${meta.color} p-4 hover:shadow-md hover:-translate-y-0.5 transition-all flex items-center gap-3`}>
