...
// Enrich every Kurs C lesson by sharing vocab + quiz + fill-in + match
// within each topic. Keeps each lesson's primary skill; just fills empty fields.
Deno.serve(async (req) => {
  try {
    ...
    for (const topic of Object.keys(byTopic)) {
      ...
      const capQuiz = sharedQuiz.slice(0, 6);
      const capFill = sharedFill.slice(0, 6);
